...
(function (Il2Cpp) {
    class Method extends NativeStruct {
        ...
        get virtualAddress() {
            // Unity 6000.3 stores MethodInfo::methodPointer at offset zero.
            // Avoid managed reflection here because stripped generic metadata
            // makes the original delegate probe dereference invalid data.
        ...
        inflate(...classes) {
            // Only read the managed generic-argument array length here.
            // Materializing every argument through classFromObject can
            // dereference stripped Unity 6000.3 metadata (commonly 0x86).
            const genericArity = method => {
                if (!method.isGeneric && !method.isInflated) {
                    return 0;
                }
                return method.object.method("GetGenericArguments").invoke().length;
            };
            if (!this.isGeneric || genericArity(this) != classes.length) {
                for (const method of this.overloads()) {
                    if (method.isGeneric && genericArity(method) == classes.length) {
...
(function (Il2Cpp) {
    let Type = class Type extends NativeStruct {
        /** */
        static get Enum() {
            // Il2CppTypeEnum is ABI-stable.  Resolving these values through
            // managed generic reflection crashes Unity 6000.3's stripped
            // metadata before the first method invocation.

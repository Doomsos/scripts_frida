...
const DEADEYE_IL2CPP_API_END_RVA = 0x3d02e9c; 
...
let deadeyeApiExportsByOrdinal;
function deadeyeRva(module, address) {
    return "0x" + address.sub(module.base).toString(16);
}
function enumerateDeadeyeApiExports() {
    if (deadeyeApiExportsByOrdinal !== undefined) {
        return deadeyeApiExportsByOrdinal;
    }
    const module = Il2Cpp.module;
    if (module == null || module.name !== "libil2cpp.so") {
        throw new Error("[Animal Rivals | rasp.bat] Expected Il2Cpp.module to be libil2cpp.so " +
    ...
    const sample = candidates
        ...
        .join(", ");
    console.log("[Animal Rivals | rasp.bat] Deadeye export scan: " +
    ...
    if (candidates.length !== DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL.length) {
        throw new Error("[Animal Rivals | rasp.bat] Protected IL2CPP export layout mismatch: " +
    ...
    if (!candidates[0].address.equals(start) ||
        ...
        throw new Error("[Animal Rivals | rasp.bat] Protected IL2CPP export boundary mismatch: " +
    ...
    console.log("[Animal Rivals | rasp.bat] Bound all 241 protected IL2CPP APIs " +
        "by validated Unity 6000.3 ordinal.");
    return candidates;
}
function resolveDeadeyeApi(ordinal) {
    const entry = enumerateDeadeyeApiExports()[ordinal];
    if (entry === undefined) {
        throw new Error("[Animal Rivals | rasp.bat] Invalid IL2CPP API ordinal " + ordinal + ".");
    }
    return entry.address;
}
const deadeyeExportResolvers = Object.create(null);
for (let ordinal = 0; ordinal < DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL.length; ordinal++) {
...
console.log("[Animal Rivals | rasp.bat] Installed 241 lazy Deadeye IL2CPP export resolvers.");

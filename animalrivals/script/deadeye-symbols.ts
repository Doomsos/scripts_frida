/*
 * Deadeye / Animal Rivals IL2CPP export resolver.
 *
 * Target build:
 *   package: com.dogelabs.deadeye
 *   version: 1335-2026.9.22-5db218
 *   Unity:   6000.3.18f1
 *
 * Deadeye renames every il2cpp_* API export to an 11-character random symbol.
 * The protected wrappers preserve the canonical API address order. This table
 * is the full 241-export order from an unobfuscated Unity 6000.3.6f1 Android
 * ARM64 reference build, validated 1:1 against the 6000.3.18f1 target.
 *
 * Load order:
 *   1. frida-il2cpp-bridge.js
 *   2. deadeye-symbols.ts
 *   3. the Animal Rivals menu script
 *
 * This file deliberately contains no randomized Deadeye export names. It
 * resolves them lazily after libil2cpp.so is loaded, verifies the exact target
 * RVA window and export count, and fails loudly if a game update changes the
 * protected wrapper layout.
 */

declare const Il2Cpp: any;

const DEADEYE_IL2CPP_API_START_RVA = 0x3cfd358;
const DEADEYE_IL2CPP_API_LAST_RVA = 0x3d02e40;
const DEADEYE_IL2CPP_API_END_RVA = 0x3d02e9c; // exclusive
const DEADEYE_RANDOM_EXPORT_NAME = /^[A-Za-z_][A-Za-z0-9_]{10}$/;

const DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL = [
    "il2cpp_init",
    "il2cpp_init_utf16",
    "il2cpp_shutdown",
...
type DeadeyeApiExport = {
    name: string;
    address: NativePointer;
};

let deadeyeApiExportsByOrdinal: DeadeyeApiExport[] | undefined;

function deadeyeRva(module: Module, address: NativePointer): string {
    return "0x" + address.sub(module.base).toString(16);
}

function enumerateDeadeyeApiExports(): DeadeyeApiExport[] {
    if (deadeyeApiExportsByOrdinal !== undefined) {
        return deadeyeApiExportsByOrdinal;
    }

    const module = Il2Cpp.module as Module;
    if (module == null || module.name !== "libil2cpp.so") {
        throw new Error(
            "[Animal Rivals | rasp.bat] Expected Il2Cpp.module to be libil2cpp.so " +
            "before resolving protected exports."
        );
    }

    const start = module.base.add(DEADEYE_IL2CPP_API_START_RVA);
    const last = module.base.add(DEADEYE_IL2CPP_API_LAST_RVA);
    const end = module.base.add(DEADEYE_IL2CPP_API_END_RVA);

    const candidates = module.enumerateExports()
        .filter(entry =>
            entry.type === "function" &&
            DEADEYE_RANDOM_EXPORT_NAME.test(entry.name) &&
            entry.address.compare(start) >= 0 &&
            entry.address.compare(end) < 0
        )
        .map(entry => ({ name: entry.name, address: entry.address }))
        .sort((left, right) => left.address.compare(right.address));

    const sample = candidates
        .slice(0, 4)
        .map(entry => entry.name + "@" + deadeyeRva(module, entry.address))
        .join(", ");

    console.log(
        "[Animal Rivals | rasp.bat] Deadeye export scan: " +
        candidates.length + " candidates in " +
        deadeyeRva(module, start) + ".." + deadeyeRva(module, end) +
        " (exclusive); first=" + (sample || "<none>")
    );

    if (candidates.length !== DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL.length) {
        throw new Error(
            "[Animal Rivals | rasp.bat] Protected IL2CPP export layout mismatch: " +
            "expected " + DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL.length +
            " functions, found " + candidates.length +
            ". The installed Animal Rivals build likely differs from " +
            "1335-2026.9.22-5db218; regenerate the ordinal map before hooking."
        );
    }

    if (!candidates[0].address.equals(start) ||
        !candidates[candidates.length - 1].address.equals(last)) {
        throw new Error(
            "[Animal Rivals | rasp.bat] Protected IL2CPP export boundary mismatch: " +
            "expected first/last RVAs " +
            "0x" + DEADEYE_IL2CPP_API_START_RVA.toString(16) + "/" +
            "0x" + DEADEYE_IL2CPP_API_LAST_RVA.toString(16) +
            ", got " + deadeyeRva(module, candidates[0].address) + "/" +
            deadeyeRva(module, candidates[candidates.length - 1].address) + "."
        );
    }

    deadeyeApiExportsByOrdinal = candidates;
    console.log(
        "[Animal Rivals | rasp.bat] Bound all 241 protected IL2CPP APIs " +
        "by validated Unity 6000.3 ordinal."
    );
    return candidates;
}

function resolveDeadeyeApi(ordinal: number): NativePointer {
    const entry = enumerateDeadeyeApiExports()[ordinal];
    if (entry === undefined) {
        throw new Error(
            "[Animal Rivals | rasp.bat] Invalid IL2CPP API ordinal " + ordinal + "."
        );
    }
    return entry.address;
}

const deadeyeExportResolvers: Record<string, () => NativePointer> =
    Object.create(null);

for (let ordinal = 0;
     ordinal < DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL.length;
     ordinal++) {
    const apiName = DEADEYE_IL2CPP_API_NAMES_BY_ORDINAL[ordinal];
    deadeyeExportResolvers[apiName] = () => resolveDeadeyeApi(ordinal);
}

Il2Cpp.$config.exports = {
    ...(Il2Cpp.$config.exports ?? {}),
    ...deadeyeExportResolvers,
};

console.log(
    "[Animal Rivals | rasp.bat] Installed 241 lazy Deadeye IL2CPP export resolvers."
);

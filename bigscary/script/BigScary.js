...
let lastTime = 0;
perviousTeleportKey = false, class SoundEntry {
  name;
  path;
  pendingPlay = false;
  failed = false;
...
Il2Cpp.perform(() => {
  ...
  function findClassLike(substr) {
    try {
      ...
      for (let a = 0; a < Il2Cpp.domain.assemblies.length; a++) {
        let img = null;
        try {
          img = Il2Cpp.domain.assemblies[a].image;
        } catch (_) {
          continue;
        }
  ...
  function ensureAudioClasses() {
    ...
    try {
      if (DirectoryClass == null) DirectoryClass = findClass("System.IO.Directory");
    } catch (_) {
    }
    try {
      if (ApplicationClass == null) ApplicationClass = findClass("UnityEngine.Application");
    } catch (_) {
    }
    try {
      if (AudioSourceClass == null) AudioSourceClass = findClass("UnityEngine.AudioSource");
    } catch (_) {
    }
    try {
      if (AudioClipClass == null) AudioClipClass = findClass("UnityEngine.AudioClip");
    } catch (_) {
    }
  }
  ensureAudioClasses();
  function decodeClipToSamples(clip) {
    try {
      ...
      if (!(total > 0) || total > 15e6) return null;
  ...
  function micFilterWrite(data, channels) {
    ...
    while (i < n && buf.offset < buf.samples.length) {
      try {
        data.set(i, data.get(i) + buf.samples[buf.offset]);
      } catch (_) {
        break;
      }
      buf.offset++;
  ...
  function sbGetRecorders() {
    ...
    try {
      ...
      console.log("[Soundboard] recorders error: " + e);
    }
    return out;
  }
  function sbEnsureLocalSrc() {
    try {
      ...
      console.log("[Soundboard] local src error: " + e);
    }
    return sbLocalSrc;
  }
  function sbPlayClip(clip) {
    try {
      let len = 0;
      try {
        len = Number(clip.method("get_length").invoke());
      } catch (_) {
      }
      if (micPlayEnabled) {
      ...
      if (src != null && !src.isNull()) {
        try {
          ...
          src.method("set_volume", 1).invoke(1);
          src.method("set_spatialBlend", 1).invoke(0);
          src.method("set_loop", 1).invoke(false);
          src.method("set_playOnAwake", 1).invoke(false);
          src.method("Play", 0).invoke();
        } catch (e) {
          console.log("[Soundboard] local play error: " + e);
        }
      }
      sbClipUntil = len > 0 ? time + len : 0;
  ...
  function sbPlayMp3(fullPath) {
    try {
      ...
      if (ReqClass && DhClass && UriClass) {
        ...
        if (!uriCtor) {
          console.log("[Soundboard] mp3: uri ctor not found");
          return;
        }
        ...
        if (!dhCtor) {
          console.log("[Soundboard] mp3: dh ctor not found");
          return;
        }
        ...
        if (!reqCtor) {
          console.log("[Soundboard] mp3: req ctor not found");
          return;
        }
      ...
      console.log("[Soundboard] mp3 error: " + e);
    }
  }
  function sbPlayWav(fullPath) {
    try {
      ...
      if (n < 44) {
        console.log("[Soundboard] wav: file too small: " + fullPath);
        return;
      }
      ...
      if (tag(0, 4) !== "RIFF" || tag(8, 4) !== "WAVE") {
        console.log("[Soundboard] wav: not a wav: " + fullPath);
        return;
      }
      ...
      while (pos + 8 <= n) {
        const id = tag(pos, 4);
        const size2 = u32(pos + 4);
        if (id === "fmt ") {
          const fmt = u16(pos + 8);
          if (fmt !== 1 && fmt !== 65534) {
            console.log("[Soundboard] wav: unsupported format " + fmt);
            return;
          }
          channels = u16(pos + 10);
      ...
      if (dataOffset < 0 || dataSize <= 0) {
        console.log("[Soundboard] wav: no data chunk: " + fullPath);
        return;
      }
      let samples;
      if (bits === 8) {
        ...
        for (let i = 0; i < frames; i++) {
          for (let c = 0; c < channels; c++) samples[out++] = (raw[dataOffset + i * channels + c] - 128) / 128;
        }
      } else {
      ...
      if (AudioClipClass == null) {
        console.log("[Soundboard] wav: AudioClip class missing");
        return;
      }
      ...
      console.log("[Soundboard] wav error: " + e);
    }
  }
  ...
  function sbFixMic() {
    try {
      ...
      if (sbLocalSrc != null && !sbLocalSrc.isNull()) {
        try {
          sbLocalSrc.method("Stop", 0).invoke();
        } catch (_) {
        }
        try {
          sbLocalSrc.method("set_clip", 1).invoke(ptr("0"));
        } catch (_) {
        }
      }
      sbClipUntil = 0;
      ...
      console.log("[Soundboard] fix mic error: " + e);
    }
  }
  function sbStopSounds() {
    try {
      ...
      console.log("[Soundboard] stop error: " + e);
    }
  }
    ...
    try {
      if (sbPending != null) {
        try {
          ...
          let result = NaN;
          try {
            result = Number(resObj.field("value__").value);
          } catch (_) {
          }
          if (result === 1) {
            ...
            let err = "?";
            try {
              err = String(req.method("get_error").invoke());
            } catch (_) {
            }
      ...
      try {
        if (DirectoryClass != null) {
          for (const c of candidates) {
            try {
              DirectoryClass.method("CreateDirectory", 1).invoke(Il2Cpp.string(c));
            } catch (_) {
            }
          }
        }
      ...
      for (const dir of candidates) {
        ...
        if (files.length > 0) {
          ...
          for (const f of files) {
            if (!seenFiles.has(f)) {
              seenFiles.add(f);
              soundboardFiles.push(f);
            }
          }
        }
      }
      soundboardFiles.sort();
      while (buttons[13].length > 3) {
        buttons[13].pop();
      }
  ...
  function spinbot() {
    getTransform(camera).method("set_rotation").invoke(Quaternion.method("Euler").invoke(0, time * 800, 0));
  }
  function OpenStaff() {
  ...
  function renderMenuText(canvasObject, text = "", color = [1, 1, 1, 1], pos = zeroVector, size2 = oneVector) {
    ...
    rectTransform.method("set_sizeDelta").invoke(size2);
  ...
  function renderMenu() {
    ...
    buttonsList.push({
      obj: homeButton,
      data: { buttonText: "<", keepOn: false, method: () => {
        currentCategory = 0;
        currentPage = 0;
      } }
    });
    const leaveButton = createObject([0.1, 0, 0.225], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
    ...
    renderMenuText(canvasObject, menuName + ` - Page <color=grey>[</color> ${currentPage + 1} <color=grey>]</color>`, textColor, [0.11, 0, 0.166], [0.95, 0.1]);
  ...
  function UpAndDown() {
    if (rightTrigger) {
      rigidbody.method("set_velocity").invoke([0, 12, 0]);
    } else if (rightGrab) {
      rigidbody.method("set_velocity").invoke([0, -12, 0]);
    }
  }
  ...
  function Velofly() {
    if (rightSecondary) {
      let forward = getTransform(rightHandTransform).method("get_forward").invoke();
      forward = Vector3.method("op_Multiply", 2).invoke(forward, flyspeed * 7 * deltaTime);
  ...
  function Platforms() {
    if (leftGrab) {
      if (platL == null) {
        ...
        platL = createObject(Vector3.method("op_Addition", 2).invoke(handTransform.method("get_position").invoke(), [0.01, -0.035, 0]), handTransform.method("get_rotation").invoke(), [0.025, 0.25, 0.3], 3, bgColor);
      }
    } else {
      if (platL != null) {
        Destroy(platL);
    ...
    if (rightGrab) {
      if (platR == null) {
        ...
        platR = createObject(Vector3.method("op_Addition", 2).invoke(handTransform.method("get_position").invoke(), [0, -0.035, 0]), handTransform.method("get_rotation").invoke(), [0.025, 0.25, 0.3], 3, bgColor);
      }
    } else {
      if (platR != null) {
        Destroy(platR);
  ...
  function Platformssphere() {
    if (leftGrab) {
      if (platL == null) {
        const handTransform = leftHandTransform;
        platL = createObject(Vector3.method("op_Addition", 2).invoke(handTransform.method("get_position").invoke(), [0, -0.035, 0]), handTransform.method("get_rotation").invoke(), [0.12, 0.12, 0.12], 0, platColor);
      }
    } else {
      if (platL != null) {
        Destroy(platL);
    ...
    if (rightGrab) {
      if (platR == null) {
        const handTransform = rightHandTransform;
        platR = createObject(Vector3.method("op_Addition", 2).invoke(handTransform.method("get_position").invoke(), [0, -0.035, 0]), handTransform.method("get_rotation").invoke(), [0.12, 0.12, 0.12], 0, platColor);
      }
    } else {
      if (platR != null) {
        Destroy(platR);
  ...
  function lagallplr() {
    if (rightGrab) {
      Destroy(PhotonNetwork.method("Instantiate", 5).invoke(Il2Cpp.string("photonvr/OnlinePlayerRig"), [400, -400, 0], identityQuaternion, 0, NULL));
      sendAllOutgoing();
    }
  }
  let skibidi = false;
  ...
  function PlrObj(OBJ) {
    const obj = GameObject.method("Find").invoke(Il2Cpp.string(OBJ));
    let pos = getTransform(GameObject.method("Find").invoke(Il2Cpp.string("GorillaPlayer"))).method("get_position").invoke();
    const offset = [0, 0, -2];
    pos = Vector3.method("op_Addition", 2).invoke(pos, offset);
    getTransform(obj).method("set_position").invoke(pos);
  }
  function CrashAll() {
    for (let i = 0; i < 1500; i++) {
      Destroy(PhotonNetwork.method("Instantiate", 5).invoke(Il2Cpp.string("Hellephant"), [400, -400, 0], identityQuaternion, 0, NULL));
    }
    sendAllOutgoing();
  }
  function CrashAllz() {
    for (let i = 0; i < 1500; i++) {
      Destroy(PhotonNetwork.method("Instantiate", 5).invoke(Il2Cpp.string("photonvr/zomb"), [400, -400, 0], identityQuaternion, 0, NULL));
      Destroy(PhotonNetwork.method("Instantiate", 5).invoke(Il2Cpp.string("photonvr/zomb2"), [400, -400, 0], identityQuaternion, 0, NULL));
    }
    sendAllOutgoing();
  }
  let size = 1;
  ...
  function strobe() {
    ...
    if (shouldbeblack) {
      PhotonVRManager.method("SetColour").invoke([0, 0, 0, 1]);
    } else {
      PhotonVRManager.method("SetColour").invoke([1, 1, 1, 1]);
    }
  }
  function strobename() {
    shouldbeblack = !shouldbeblack;
  ...
  const buttons = [
    [
      ...
      new ButtonInfo({ buttonText: "Custom Soundboard", toolTip: "Opens the Soundboard page", method: () => {
        scanSoundboard();
        currentCategory = 13;
        currentPage = 0;
      }, keepOn: false }),
    ...
    [
      ...
      new ButtonInfo({ buttonText: "Reset fly speed", toolTip: "Sets the fly speed to 8", method: () => flyspeed = 8, keepOn: false }),
      new ButtonInfo({
        ...
        method: () => {
          ...
          switch (themeIndex) {
            case 0:
              bgColor = [1, 0.5, 0, 1];
              textColor = [1, 0.7450981, 0.4901961, 1];
              buttonColor = [0.666, 0.333, 0, 1];
              buttonPressedColor = [0.333, 0.15, 0, 1];
              break;
            case 1:
              bgColor = [1, 0, 0, 1];
              textColor = [1, 1, 1, 1];
              buttonColor = [0, 0, 0, 1];
              buttonPressedColor = [1, 0, 0, 1];
              break;
            case 2:
              bgColor = [0, 1, 0, 1];
              textColor = [1, 1, 1, 1];
              buttonColor = [0, 0, 0, 1];
              buttonPressedColor = [0, 1, 0, 1];
              break;
            case 3:
              bgColor = [0, 0, 1, 1];
              textColor = [1, 1, 1, 1];
              buttonColor = [0, 0, 0, 1];
              buttonPressedColor = [0, 0, 1, 1];
              break;
            case 4:
              bgColor = [0.5, 0, 0.5, 1];
              textColor = [1, 0.9, 1, 1];
              buttonColor = [0.25, 0, 0.25, 1];
              buttonPressedColor = [0.7, 0, 0.7, 1];
              break;
            case 5:
              bgColor = [0, 0.7, 0.7, 1];
              textColor = [1, 0.2, 0.8, 1];
              buttonColor = [0, 0.3, 0.3, 1];
              buttonPressedColor = [1, 0, 0.7, 1];
              break;
            case 6:
              bgColor = [0.9, 0.7, 0.1, 1];
              textColor = [0, 0, 0, 1];
              buttonColor = [0.2, 0.2, 0.2, 1];
              buttonPressedColor = [1, 0.84, 0, 1];
              break;
            case 7:
              bgColor = [0.7, 0.9, 1, 1];
              textColor = [0.1, 0.2, 0.4, 1];
              buttonColor = [0.5, 0.7, 0.9, 1];
              buttonPressedColor = [0.2, 0.4, 0.8, 1];
              break;
            case 8:
              bgColor = [0.6, 0, 0, 1];
              textColor = [1, 0.7, 0.2, 1];
              buttonColor = [0.2, 0, 0, 1];
              buttonPressedColor = [1, 0.3, 0, 1];
              break;
            case 9:
              bgColor = [0.05, 0.05, 0.1, 1];
              textColor = [0.6, 0.8, 1, 1];
              buttonColor = [0.1, 0.1, 0.2, 1];
              buttonPressedColor = [0.4, 0.6, 1, 1];
              break;
            case 10:
              bgColor = [0.125, 0.125, 0.125, 1];
              textColor = [1, 1, 0.9, 1];
              buttonColor = [0.111, 0.111, 0.222, 1];
              buttonPressedColor = [0.111, 0.111, 0.67, 1];
              break;
            case 11:
              bgColor = [0.055, 0.018, 0.095, 0.98];
              textColor = [1, 1, 1, 1];
    ...
    [
      ...
      new ButtonInfo({ buttonText: "Iron Man [G]", method: () => {
        if (leftGrab) {
          ...
          const leftForce = Vector3.method("op_Multiply", 2).invoke(leftRightVector, -15 * deltaTime);
      ...
      }, toolTip: "Use your grip buttons to fly like ironman" }),
      new ButtonInfo({ buttonText: "Dash", method: () => {
        if (rightPrimary && !previousDash) {
          ...
          const leftForce = Vector3.method("op_Multiply", 2).invoke(leftRightVector, 10);
          rigidbody.method("AddForce", 2).invoke(leftForce, 2);
        }
      ...
      new ButtonInfo({ buttonText: "Low Gravity", toolTip: "Makes gravity lower on your character", method: () => {
      ...
      new ButtonInfo({ buttonText: "High Gravity", toolTip: "Makes gravity higher on your character", method: () => {
        const force = Vector3.method("op_Multiply", 2).invoke(Vector3.field("upVector").value, deltaTime * (-6.66 / deltaTime));
        rigidbody.method("AddForce", 2).invoke(force, 5);
      } }),
      new ButtonInfo({ buttonText: "Reverse Gravity", toolTip: "Reverses your gravity", method: () => {
        const force = Vector3.method("op_Multiply", 2).invoke(Vector3.field("upVector").value, deltaTime * (15.6 / deltaTime));
        rigidbody.method("AddForce", 2).invoke(force, 5);
      } }),
      new ButtonInfo({ buttonText: "No Clip [T]", toolTip: "Go through objects using right trigger", method: () => Noclip() }),
      new ButtonInfo({ buttonText: "SpeedBoost", toolTip: "Become Fast\n(Max Jump Speed)", method: () => GTPlayer.field("maxJumpSpeed").value = 9 }),
      new ButtonInfo({ buttonText: "Faster speedBoost", toolTip: "Become Faster\n(Max Jump Speed)", method: () => GTPlayer.field("maxJumpSpeed").value = 20 }),
      new ButtonInfo({ buttonText: "Very High speedBoost", toolTip: "Become even faster\n(Max Jump Speed)", method: () => GTPlayer.field("maxJumpSpeed").value = 50 })
    ],
    [
      // Misc Mods
      ...
      new ButtonInfo({
        ...
        method: () => {
          ...
          if (elapsed < targetDelta) {
            const sleepMs = Math.floor((targetDelta - elapsed) * 1e3);
            if (sleepMs > 0)
    ...
    [
      ...
      new ButtonInfo({ buttonText: "Long Arms", toolTip: "Makes your rig big", method: () => {
        size = 1.3;
        scalearms();
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Scale Arms", toolTip: "Right Trigger = longer arms, Left Trigger = Shorter Arms", method: () => scalearms() }),
      new ButtonInfo({ buttonText: "Reset long arms", toolTip: "Resets size", method: () => {
        size = 1;
        scalearms();
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Spin bot", toolTip: "Spins your body on y axis", method: () => spinbot(), keepOn: true }),
      new ButtonInfo({ buttonText: "Spaz rig", toolTip: "Spins your body on all axis", method: () => {
        const randomX = Math.random() * 360;
        const randomY = Math.random() * 360;
        const randomZ = Math.random() * 360;
        const cam = GameObject.method("Find").invoke(Il2Cpp.string("MainCamera"));
        if (!cam) return;
        cam.method("get_transform").invoke().method("set_rotation").invoke(Quaternion.method("Euler").invoke(randomX, randomY, randomZ));
      }, keepOn: true })
    ],
    [
      // OP Mods
    ...
    [
      ...
      new ButtonInfo({ buttonText: "lvl 1 monster gun", toolTip: "teleports lvl 1 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV1");
      }, keepOn: true }),
      new ButtonInfo({ buttonText: "lvl 2 monster gun", toolTip: "teleports lvl 2 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV2");
      }, keepOn: true }),
      new ButtonInfo({ buttonText: "lvl 3 monster gun", toolTip: "teleports lvl 3 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV3");
      }, keepOn: true }),
      new ButtonInfo({ buttonText: "lvl 4 monster gun", toolTip: "teleports lvl 4 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV4");
      }, keepOn: true }),
      new ButtonInfo({ buttonText: "lvl 5 monster gun", toolTip: "teleports lvl 5 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV5");
      }, keepOn: true }),
      new ButtonInfo({ buttonText: "lvl 6 monster gun", toolTip: "teleports lvl 6 monster to gun pointer", method: () => {
        PhotonNetwork.method("SetMasterClient").invoke(PhotonNetwork.method("get_LocalPlayer").invoke());
        ObjGun("NAV6");
      }, keepOn: true })
    ],
    ...
    [
      ...
      new ButtonInfo({ buttonText: "Become J0kerModZ", toolTip: "Changes the player name and color", method: () => {
        PhotonNetwork.method("set_NickName").invoke(Il2Cpp.string("J0KERMODZ"));
        PhotonVRManager.method("SetColour").invoke([0.9, 0.5, 0.9, 1]);
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Become JaSC", toolTip: "Changes the player name and color", method: () => {
        PhotonNetwork.method("set_NickName").invoke(Il2Cpp.string("JASC"));
        PhotonVRManager.method("SetColour").invoke([0, 0.9, 0.3, 1]);
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Become Wireless", toolTip: "Changes the player name and color", method: () => {
        PhotonNetwork.method("set_NickName").invoke(Il2Cpp.string("WIRELESS"));
        PhotonVRManager.method("SetColour").invoke([0.5, 0, 0.9, 1]);
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Fix Player", toolTip: "Fixes rotation", method: () => {
        const plr = GameObject.method("Find").invoke(Il2Cpp.string("GorillaPlayer"));
        const trans = getTransform(plr);
        trans.method("set_rotation").invoke([0, 0, 0, 0]);
      }, keepOn: false }),
      new ButtonInfo({ buttonText: "Upside down player", toolTip: "Reverses your gravity and makes you upside down", method: () => {
        const force = Vector3.method("op_Multiply", 2).invoke(Vector3.field("upVector").value, deltaTime * (15.6 / deltaTime));
        rigidbody.method("AddForce", 2).invoke(force, 5);
        const plr = GameObject.method("Find").invoke(Il2Cpp.string("GorillaPlayer"));
        const trans = getTransform(plr);
        trans.method("set_rotation").invoke([0, 0, 180, 0]);
      } })
    ],
    [
      //Testing
      new ButtonInfo({
        ...
        method: () => {
          if (rightGrab) {
            const cn = renderGun(), co = cn.ray, collider = co.method("get_collider").invoke();
            if (rightTrigger && !perviousTeleportKey) {
              ...
              for (let cr = 0; cr < getComponentsResult.length; cr++) {
                const cs = getComponentsResult.get(cr), ct = cs.method("GetType", 0).invoke().method("get_Name").invoke().toString();
                console.error(ct);
              }
      ...
      new ButtonInfo({
        ...
        method: () => {
          if (rightGrab) {
            const cm = renderGun(), cn = cm.gunPointer;
            if (rightTrigger && !perviousTeleportKey) {
    ...
    [
      ...
      new ButtonInfo({ buttonText: "Refresh Soundboard", toolTip: "refreshes sounds", method: () => {
        scanSoundboard();
        reloadMenu();
      }, keepOn: false }),
      new ButtonInfo({
        buttonText: "Play on Mic",
        toolTip: "plays on microphone",
        enableMethod: () => {
          micPlayEnabled = true;
          micMixBuffer = null;
        },
  ...
  LateUpdate.implementation = function() {
    ...
    buttons.flat().filter((button) => button.enabled).forEach((button) => {
      if (button.method) {
        try {
          button.method();
        } catch (error) {
